...
public class TelemetryConsumer {
    ...
    public Uni<Void> consume(Message<byte[]> message) {
        ...
        Log.debugf("TelemetryConsumer received message on topic: %s", topic);
        ...
        return telemetryService.writeTelemetry(deviceName, "sensor_data", fields)
                ...
                .onFailure().invoke(e -> Log.errorf("Failed to write telemetry from %s (payload: %s): %s", deviceName, payload, e.getMessage()))
