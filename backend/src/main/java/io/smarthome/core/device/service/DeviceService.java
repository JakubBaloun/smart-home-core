...
import io.smarthome.core.device.service.mapper.Z2MDeviceMapper;
...
public class DeviceService {
        ...
        return Uni.createFrom().item(payloads)
                ...
                .chain(activeIeeeAddresses -> {
                    ...
                    .invoke(() -> {
                        ...
                        Log.infof("Device sync completed.");
                    });
                }).replaceWithVoid();
    }

    public Uni<Void> updateAvailability(String friendlyName, boolean available) {
        return sessionFactory.withTransaction(session ->
                        deviceRepository.updateAvailability(friendlyName, available, session))
                .invoke(updated -> {
                    if (updated == 0) {
                        Log.debugf("Availability update for unknown device '%s' ignored", friendlyName);
                    } else {
                        Log.infof("Device '%s' is now %s", friendlyName, available ? "online" : "offline");
                    }
                })
                .replaceWithVoid();
    }
    ...
    public Uni<List<Device>> getAllDevices() {
        return Uni.createFrom().voidItem()
                ...
                .invoke((devices) -> Log.debugf("Retrieved %d devices from the database", devices.size()))
                .onFailure()
    ...
    public Uni<Device> getDeviceById(Long id) {
        return Uni.createFrom().voidItem()
                ...
                .invoke((device) -> Log.debugf("Device with id %d retrieved successfully", id))
                .onFailure()
