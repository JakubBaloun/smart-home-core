...
public class TelemetryServiceTest {
    ...
    @Test
    void testWriteTelemetry_coercesIntegersToDouble() {
        // GIVEN - Jackson parses JSON numbers without decimal point as Integer/Long
        Map<String, Object> fields = Map.of("temperature", 25, "battery", 100L, "contact", true);

        // WHEN
        telemetryService.writeTelemetry("device-123", "sensor_data", fields)
                .await().indefinitely();

        // THEN - numbers must be written as floats (no 'i' suffix), booleans preserved
        ArgumentCaptor<Point> captor = ArgumentCaptor.forClass(Point.class);
        verify(writeApi).writePoint(eq("telemetry"), eq("smart-home"), captor.capture());
        String lineProtocol = captor.getValue().toLineProtocol();
        assertFalse(lineProtocol.contains("temperature=25i"), lineProtocol);
        assertFalse(lineProtocol.contains("battery=100i"), lineProtocol);
        assertTrue(lineProtocol.contains("contact=true"), lineProtocol);
    }

    @Test
    void testWriteTelemetry_skipsNonNumericFields() {
        // GIVEN
        Map<String, Object> fields = Map.of("temperature", 21.5, "state", "ON");
        ...
        verify(writeApi).writePoint(eq("telemetry"), eq("smart-home"), captor.capture());
        String lineProtocol = captor.getValue().toLineProtocol();
        assertFalse(lineProtocol.contains("state="), lineProtocol);
        assertTrue(lineProtocol.contains("temperature=21.5"), lineProtocol);
    }
